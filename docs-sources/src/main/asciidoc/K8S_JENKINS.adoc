...
```bash
$ ./tools/minikube-helper.sh download-gcloud
```
...
* `./tools/minikube-helper.sh setup-tools-infra-vsphere` for a cluster deployed on VSphere
* `./tools/minikube-helper.sh setup-tools-infra-gce` for a cluster deployed to GCE
...
image::{jenkins-root-docs}/kubo_update_git.png[caption="Update the `git` credentials with your data"]

- Add `k8s-token` credential with the `token`

image::{jenkins-root-docs}/kubo_token.png[caption="Add the `k8s-token` text credential with the k8s access `token`"]

- Click `Manage Jenkins` and pick `Script Console`
...
* Put your `KUBERNETES_API:KUBERNETES_PORT` or `kubernetes` here
** `PAAS_TEST_API_URL`
** `PAAS_STAGE_API_URL`
** `PAAS_PROD_API_URL`
...
* Set `k8s-token` value to these ids
