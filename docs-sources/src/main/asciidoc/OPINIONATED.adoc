...
IMPORTANT: In the section below we're showing you a full flow of the demo apps.
It doesn't mean that your applications are required to have the same dependencies
such as e.g. `Eureka`.
...
- For PHP (https://github.com/spring-cloud-samples/cf-php-example[example project]):
    ...
    * `target` is assumed to be the output folder. Put it to `.gitignore`
- For NodeJS (https://github.com/spring-cloud-samples/spring-cloud-contract-nodejs/tree/sc-pipelines[example project]):
...
- For .Net (https://github.com/spring-cloud-samples/AspNetCoreExample[example project]):
    ...
    * `dotnet msbuild /nologo /t:CFPPublish /p:Configuration=Release` is called to publish
    ZIP with self contained DLL, together with all manifests and deployment files
    ...
    * `dotnet msbuild /nologo /t:CFPStubIds` npm task that echos stub runner ids
    * `dotnet msbuild /nologo /t:CFPApiCompatibilityTest` is executed for api compatibility tests
