...
`Concourse` combination) it's enough to execute

```bash
$ ./gradlew customize
```

and answer a couple of questions. That way whole files and its pieces
will get removed / updated accordingly. For example in the aforementioned
case all `Kubernetes` and `Jenkins` configuration / folders / pieces of code in
