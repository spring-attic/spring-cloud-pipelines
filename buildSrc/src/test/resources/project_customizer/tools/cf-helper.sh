...
PAAS_TEST_SPACE_PREFIX="${PAAS_TEST_SPACE_PREFIX:-pcfdev-test}"
...
case $1 in
	...
	setup-prod-infra)
		...
		cf push "github-eureka" -p "build/eureka.jar" -n "github-eureka" -b "https://github.com/cloudfoundry/java-buildpack.git#v4.12" -m "${EUREKA_MEMORY}" -i 1 --no-manifest --no-start
