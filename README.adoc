...
- <<fork,Fork repos and deploy infra to Artifactory>>
- <<start,Start Jenkins and Artifactory>>
- <<pcfdev,[OPTIONAL] Start PCF Dev (if you don't want to use an existing one)>>
- <<env,[OPTIONAL] Setup Jenkins env vars (if you want to use the demo defaults and you're using Docker Machine
...
- <<settings,[OPTIONAL] Add `settings.xml` for Jenkins' master (you can skip this if you want to use our defaults)>>
- <<seed,Run the seed job>>
- <<pipeline,Run the `github-webhook` pipeline>>

=== Fork repos and deploy infra to Artifactory[[fork]]
...
image::docs/credentials_example.png[caption="Step 4: ", title="Fill out the user / password and provide the `git` credential ID (in this example `cf-test`)"]

{nbsp}
{nbsp}

TIP: The following section is just for informational purposes. You can skip it if you just want
to run the demo
...
image::docs/seed.png[caption="Step 3: ", title="Provide the `REPOS` parameter with URLs of your forks (you'll have more properties than the ones in the screenshot)"]
...
image::docs/seed_views.png[caption="Step 1: ", title="Click the 'github-webhook' view"]

{nbsp}
{nbsp}

image::docs/pipeline_run.png[caption="Step 2: ", title="Run the pipeline"]

{nbsp}
{nbsp}

image::docs/pipeline_run_props.png[caption="Step 3: ", title="You can set some properties (just click 'Build' to proceed) "]
...
image::docs/pipeline_manual.png[caption="Step 4: ", title="Click the manual step to go to stage (remember about killing the apps on test env)"]
...
image::docs/pipeline_finished.png[caption="Step 5: ", title="The full pipeline should look like this"]
...
image::docs/jdk.png[caption="Step 4: ", title="Fill out JDK Installation with path to your JDK"]
