...
|PIPIELINE_VERSION | What should be the version of the pipeline (ultimately also version of the jar) | jdk8
|======================
...
image::docs/groovy_token.png[caption="Step 1: ", title="Click 'Manage Jenkins'"]
...
image::docs/git.png[caption="Step 4: ", title="Fill out JDK Installation with `/usr/lib/jvm/java-8-openjdk-amd64`"]
...
Docker, docker, docker... The problem is that for some reason, only in Docker, the execution of
