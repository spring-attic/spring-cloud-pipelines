...
SpinnakerPipelineDefaults defaults = new SpinnakerPipelineDefaults(binding.variables)
...
SpinnakerBashFunctions bashFunctions = new SpinnakerBashFunctions(gitName, gitEmail, gitUseSshKey)
...
SpinnakerRepoType repoType = SpinnakerRepoType.from(toolsRepo)
...
Closure<String> downloadTools = { String repoUrl ->
	...
	if (repoType == SpinnakerRepoType.TARBALL) {
	...
	dsl.job("${projectName}-build") {
		...
		wrappers {
			...
			credentialsBinding {
				...
				if (!gitUseSshKey) usernamePassword(SpinnakerPipelineDefaults.GIT_USER_NAME_ENV_VAR, SpinnakerPipelineDefaults.GIT_PASSWORD_ENV_VAR, gitCredentials)
			}
		}
		jdk(jdkVersion)
		scm {
 ...
 */
@CompileStatic
class SpinnakerPipelineDefaults {
	...
	SpinnakerPipelineDefaults(Map<String, String> variables) {
...
@CompileStatic
enum SpinnakerRepoType {
	TARBALL, GIT

	static SpinnakerRepoType from(String string) {
...
@CompileStatic
class SpinnakerBashFunctions {

	private final boolean gitUseSsh
	private final String gitUser
	private final String gitEmail

	SpinnakerBashFunctions(String gitUser, String gitEmail, boolean gitUseSsh) {
	...
	String setupGitCredentials(String repoUrl) {
		...
		return """\
				...
				echo "https://\$${SpinnakerPipelineDefaults.GIT_USER_NAME_ENV_VAR}:\$${SpinnakerPipelineDefaults.GIT_PASSWORD_ENV_VAR}@${host}" > \${tmpDir}/gitcredentials
			"""
	}
}
