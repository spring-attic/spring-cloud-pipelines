...
@test "should run the build-api-compatibility-check task with passed pipeline version" {
	...
	run "${ROOT_FOLDER}"/tools/concourse/tasks/build-api-compatibility-check/task.sh
	...
	PIPELINE_VERSION_PRESENT="$( echo "${KEYVAL}" | grep -q "PASSED_PIPELINE_VERSION=1.2.3.FOO"; echo $? )"
	assert_equal "${PIPELINE_VERSION_PRESENT}" "0"
