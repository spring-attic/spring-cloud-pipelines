...
@test "should run the prod-rollback task" {
	export PROJECT_NAME="foo"
	export PASSED_PIPELINE_VERSION="1.2.3.FOO"
	cd "${ROOT_FOLDER}"

	run "${ROOT_FOLDER}"/tools/concourse/tasks/prod-rollback/task.sh

	assert_success
	assert_output --partial "git push --delete origin prod/foo/1.2.3.FOO"
}
