...
@test "should run the generate-version task with passed pipeline version" {
	cd "${ROOT_FOLDER}"
	mkdir -p "updated-version"

	run "${ROOT_FOLDER}"/tools/concourse/tasks/generate-version/task.sh

	assert_success
	KEYVAL="$( cat "${ROOT_FOLDER}"/keyvalout/keyval.properties )"
	PASSED_PIPELINE_VERSION_PRESENT="$( echo "${KEYVAL}" | grep -q "PASSED_PIPELINE_VERSION=1.0.0.M1-"; echo $? )"
	assert_equal "${PASSED_PIPELINE_VERSION_PRESENT}" "0"
	assert [ -e "${ROOT_FOLDER}/updated-version/version" ]
}
